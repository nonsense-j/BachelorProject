module Ref {
  ...
  active component ResCapExecutor {
    ...
    @ Port for receiving action status
    async input port resCapActStatusin : ResCapActStatus

    output port resCapStartout : ResCapAct

    output port resCapMoveout : ResCapAct
    ...
    event INIT_SUCC() \
      ...
      format "[ResCap] Stategy initializes successfully. Specification is located at : [Ref/ResCapMission/Config/firefighting.spec]." \
    ...
    event SHOW_TANSITION(
                          ...
                          region_resCap: string @< current region real name
                        ) \
