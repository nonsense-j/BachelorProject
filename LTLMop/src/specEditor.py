...
class AnalysisResultsDialog(wx.Dialog):
    ...
    def appendLog(self, text, color="WHITE"):
...
class MapShow(wx.Dialog):
    """
    just show the map for screenshotting
    """
    ...
    def __init__(self, parent, *args, **kwds):
        ...
        self.set_decomposed = False
        self.parent = parent

    def __set_properties(self):
        # begin wxGlade: MapDialog.__set_properties
        self.SetTitle("Take a Screenshot to Build Your Own Map")
        self.SetSize((1000, 729))
        self.panel_2.SetScrollRate(10, 10)
        # end wxGlade
    ...
    def drawMap(self, event):
        if self.set_decomposed:
            mapRenderer.drawMap(self.panel_2, self.parent.decomposedRFI, scaleToFit=False)
        else:
            mapRenderer.drawMap(self.panel_2, self.parent.proj.rfi, scaleToFit=False)
...
class SpecEditorFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwds):
        ...
        global MENU_BUILDCONFIG; MENU_BUILDCONFIG = wx.NewId()
        global MENU_BUILDFPRIME; MENU_BUILDFPRIME = wx.NewId()
        global MENU_BUILDFPRIMEINC; MENU_BUILDFPRIMEINC = wx.NewId()
        global MENU_CLEANFPRIME; MENU_CLEANFPRIME = wx.NewId()

        global MENU_CONFIGFPRIME; MENU_CONFIGFPRIME = wx.NewId()
        ...
        wxglade_tmp_menu.AppendSeparator()
        wxglade_tmp_menu_sub = wx.Menu()
        wxglade_tmp_menu_sub.Append(MENU_BUILDFPRIME, "Override Mode", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu_sub.Append(MENU_BUILDFPRIMEINC, "Incremental Mode", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu.AppendMenu(MENU_BUILDCONFIG, "Build System in F Prime", wxglade_tmp_menu_sub, "")
        wxglade_tmp_menu.Append(MENU_CLEANFPRIME, "Clean System in F Prime", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu.Append(MENU_CONFIGFPRIME, "Set F Prime Root Path", "", wx.ITEM_NORMAL)
        ...
        self.button_omap = wx.Button(self.panel_1, wx.ID_ANY, "Show Origional Map")
        self.button_dmap = wx.Button(self.panel_1, wx.ID_ANY, "Show Decomposed Map")
        ...
        self.Bind(wx.EVT_MENU, self.onMenuBuildFPrime, id=MENU_BUILDFPRIME)
        self.Bind(wx.EVT_MENU, self.onMenuBuildFPrimeInc, id=MENU_BUILDFPRIMEINC)
        self.Bind(wx.EVT_MENU, self.onMenuCleanFPrime, id=MENU_CLEANFPRIME)
        self.Bind(wx.EVT_MENU, self.onMenuConfigFPrime, id=MENU_CONFIGFPRIME)
        ...
        self.Bind(wx.EVT_BUTTON, self.onMapShow, self.button_omap)
        self.Bind(wx.EVT_BUTTON, self.onMapShowDecomposed, self.button_dmap)
        ...
        self.text_ctrl_spec.MarkerDefine(MARKER_SAFE,wx.stc.STC_MARK_ARROW,"white","SKYblue")
    ...
    def initializeNewSpec(self):
        ...
        self.button_omap.Enable(False)
        self.button_dmap.Enable(False)
        ...
        self.dirty = False
        # "{}/lib/FPBuilder/output".format(self.proj.ltlmop_root)
        self.fprime_root = ""
        self.fprime_ref_root = ""
    ...
    def __do_layout(self):
        ...
        # sizer_7.Add((5, 20), 0, 0, 0)
        sizer_17.Add(self.button_omap, 0, wx.TOP, 5)
        sizer_17.Add((5, 20), 0, 0, 0)
        sizer_17.Add(self.button_dmap, 0, wx.TOP, 5)
        sizer_5.Add(sizer_7, 0, wx.LEFT | wx.EXPAND, 4)
        sizer_5.Add(sizer_17, 0, wx.LEFT | wx.EXPAND, 4)
    ...
    def onMapShow(self, event): # wxGlade: SpecEditorFrame.<event_handler>
        """
        Show the original map
        """
        self.mapShow.set_decomposed = False
        self.mapShow.Show()

        event.Skip()

    def onMapShowDecomposed(self, event): # wxGlade: SpecEditorFrame.<event_handler>
        """
        Show the map with overlayed regions so that the user can select a region name visually.
        """
        self.mapShow.set_decomposed = True
        self.mapShow.Show()
    ...
    def updateFromRFI(self):
        ...
        if self.mapShow is not None:
            self.mapShow.Destroy()

        self.mapDialog = MapDialog(frame_1, frame_1)
        self.mapShow = MapShow(frame_1, frame_1)
        frame_1.button_map.Enable(True)
        frame_1.button_omap.Enable(True)
    ...
    def openFile(self, filename):
        ...
        if self.decomposedRFI is None and self.proj.regionMapping is not None:
            ...
            frame_1.button_dmap.Enable(True)
    ...
    def onMenuBuildFPrime(self, event): # wxGlade: SpecEditorFrame.<event_handler>
        """ Build the FPrime System with current experiment configuration in override mode. """

        if self.fprime_root == "":
            wx.MessageBox("Cannot find F Prime.\nPlease set the root path of F Prime before building.", "Error",
                        style = wx.OK | wx.ICON_ERROR)
            return
        if self.proj.project_basename is None:
            wx.MessageBox("Please open or save your project.", "Error",
                          style=wx.OK | wx.ICON_ERROR)
            return
        if not (self.proj.project_basename[0]).isupper():
            wx.MessageBox("Project name must be capitalized.", "Error",
                          style=wx.OK | wx.ICON_ERROR)
            return

        if not self.proj.compile_options["use_region_bit_encoding"]:
            wx.MessageBox("Execution in F Prime requires bit-vector region encoding.\nPlease enable it and recompile.", "Error",
                        style = wx.OK | wx.ICON_ERROR)
            return
        ...
        self.appendLog("F Prime Root is found at : " + self.fprime_root + "\n", "SKYBLUE")
        self.appendLog("[Override Mode]Starting to build fprime system for simulation...\n", "SKYBLUE")
        builder = FPBuild.FPBuilder(self.proj.project_basename, self.fprime_ref_root)
        builder.buildAll(self.proj.all_sensors, self.proj.all_actuators, self.proj.all_customs)
        builder.copyConfig(self.proj.project_root)
        self.appendLog("Finished building fprime system for simulation...\n", "SKYBLUE")

    def onMenuBuildFPrimeInc(self, event): # wxGlade: SpecEditorFrame.<event_handler>
        """ Build the FPrime System with current experiment configuration in incremental mode. """
        ...
        self.appendLog("F Prime Root is found at : " + self.fprime_root + "\n", "SKYBLUE")
        self.appendLog("[Incremental Mode]Starting to build fprime system for simulation...\n", "SKYBLUE")
        builder = FPBuild.FPBuilder(self.proj.project_basename, self.fprime_ref_root)
        builder.buildAll(self.proj.all_sensors, self.proj.all_actuators, self.proj.all_customs, add_write=True, init_clean=False)
        builder.copyConfig(self.proj.project_root)
        self.appendLog("Finished building fprime system for simulation...\n", "SKYBLUE")

    def onMenuCleanFPrime(self, event):  # wxGlade: SpecEditorFrame.<event_handler>
        """ Clean the FPrime System. """
        ...
        self.appendLog("F Prime Root is found at : " + self.fprime_root + "\n", "SKYBLUE")
        self.appendLog("Starting to clean fprime system ...\n", "SKYBLUE")
        builder = FPBuild.FPBuilder(self.proj.project_basename, self.fprime_ref_root)
        builder.cleanAll()
        self.appendLog("Cleaning procedure finished...\n", "SKYBLUE")


    def onMenuConfigFPrime(self, event):  # wxGlade: SpecEditorFrame.<event_handler>
        """
        Set the root path of f prime
        """

        root_name = wx.DirSelector(defaultPath="..")
        if root_name == "":
            return
        if not os.path.isfile(root_name + "/Ref/CMakeLists.txt"):
            wx.MessageBox("ERROR: [" + root_name + "] is not the root of F Prime. Please Reconfig the Root Path.", "Error",
                          style=wx.OK | wx.ICON_ERROR)
            return

        self.appendLog("F Prime Root is found at : " + root_name + "\n", "YELLOW")
        self.fprime_root = root_name
        self.fprime_ref_root = os.path.join(self.fprime_root, "Ref")
    ...
    def onMenuAnalyze(self, event): # wxGlade: SpecEditorFrame.<event_handler>
        ...
        if realizable:
            ...
            self.analysisDialog.appendLog('\n'.join(output_lines), "WHITE")
            if nonTrivial:
                self.analysisDialog.appendLog("\nSynthesized automaton is non-trivial.", "WHITE")
            else:
...
class RedirectText:
    ...
    def write(self,string):
        self.out.BeginTextColour("WHITE")
